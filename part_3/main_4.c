...
int main(int argc, char ** argv)
{
	...
	dataType_t * data0_half1 = (dataType_t *)sds_alloc(sizeof(dataType_t)*dim*size/2);
	dataType_t * data0_half2 = (dataType_t *)sds_alloc(sizeof(dataType_t)*dim*size/2);
	...
	dataType_t *data3 = (dataType_t *)sds_alloc(sizeof(dataType_t)*dim*size);
	...
	clock_gettime(0, &timerStart_sw_accel);


	myFunc_Accel_4(size,dim,threshold,data0,data1,data3);

	clock_gettime(0, &timerStop_sw_accel);
...
//		assert(data3[i]==data2[i]);
		int diff= data3[i]-data2[i];
		if (abs(diff)>0.001)
		{
			printf("Error data3[%d]=%f and data2[%d]=%f ",i,data3[i],i,data2[i]);
			break;
		}
	}
